...
function recalculate(modelName, duration) {
  ...
  costLabel.innerHTML = newTotal.toFixed(2);
}
/****************** model button logic ******************/
...
function changeDuration() {
  ...
  if (!Number.isNaN(newDuration) && newDuration !== null) {
    duration = parseFloat(newDuration); // Update global duration variable
    durationText.innerHTML = newDuration;
    recalculate(modelName, duration);
  } else {
    alert("Error. Please enter a valid number (days).");
  }
}
